...
def main() -> None:
    ...
    lines.append(
        "This report consolidates the current paper-main evidence: Format-SFT baseline, seed42 main checkpoint, seed43 confirmation, and fixed-corpus control. "
        "It does not introduce new training, new data, or new reward variants.\n\n"
    )
    ...
    lines.append(
        "The stable claim is that the DAG-IG GRPO recipe improves over Format-SFT in a two-stage offline retrieval setting. "
        "Seed42 improves strict success from `42.9%` to `49.0%` on dev and from `34.4%` to `40.6%` on test. "
        "Seed43 confirms the recipe with `49.0%` dev and `39.1%` test strict. "
        "The fixed-corpus control is train-healthy and reaches `50.0%` dev strict, but does not replace seed42 because test strict is `39.1%`.\n\n"
    )
    lines.append("## 5. Bottleneck\n\n")
    lines.append(
        "The remaining bottleneck is not format or answer leakage: format is near-perfect and answer-in-query is zero in the main runs. "
        "The dominant errors remain retrieval misses and retrieval-hit-answer-wrong cases. "
        "For seed42, dev/test retrieval misses are `42 / 31`, and hit-answer-wrong cases are `8 / 7`. "
        "For the fixed-corpus control, dev/test retrieval misses are `42 / 32`, and hit-answer-wrong cases are `7 / 7`.\n\n"
    )
    lines.append("## 6. Decision\n\n")
    lines.append(
        "Keep `outputs/dagig_paper_main_v1/checkpoints/paper_main_v1_two_stage_stage1loss_kl01_scale60_s320/checkpoint-60` as the current main checkpoint. "
        "Use seed43 as confirmation and the fixed-corpus run as a robustness/control ablation. "
        "The next paper-facing step is not another same-recipe GRPO run; it is writing the method/result narrative around node-level DAG-IG credit, then deciding whether a targeted retrieval or reader mechanism is necessary for a stronger final result.\n"
    )
