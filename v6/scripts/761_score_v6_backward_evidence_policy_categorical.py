#!/usr/bin/env python3
"""Score one explicit-categorical evidence adapter on frozen action groups."""
...
def field_token_mask(tokenizer: Any, completion: str) -> tuple[list[int], list[int]]:
    ...
    if sum(mask) != 1:
        raise ValueError("categorical action must score exactly one token")
...
def main() -> None:
    ...
    if freeze.get("protocol_version") != "dagig_v6_backward_evidence_explicit_categorical_actions_v1":
        raise ValueError("evidence scorer requires the explicit categorical protocol")
    ...
    data_key = "categorical_train_data" if args.partition == "policy_train" else "categorical_internal_data"
    data_path = Path(freeze["input_paths"][data_key])
    if sha256(data_path) != freeze["input_hashes"][data_key]:
        raise ValueError("categorical evidence action data changed")
