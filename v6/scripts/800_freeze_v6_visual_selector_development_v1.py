#!/usr/bin/env python3
"""Freeze the visual-selector development audit under the full downstream DAG."""
...
def main() -> None:
    ...
    parser.add_argument("--visual_value_freeze", type=Path, required=True)
    parser.add_argument("--auditor", type=Path, required=True)
    parser.add_argument("--output_dir", type=Path, required=True)
    args = parser.parse_args()

    query_freeze_path = args.visual_value_freeze.resolve()
    ...
    full_query_freeze = read_json(Path(query_freeze["input_paths"]["query_value_freeze"]))
    evidence_protocol_path = Path(full_query_freeze["input_paths"]["evidence_protocol_freeze"])
    ...
    result = {
        "decision": "DAGIG_V6_VISUAL_SELECTOR_DEVELOPMENT_PROTOCOL_FROZEN",
        "protocol_version": "dagig_v6_visual_selector_development_v1",
        "selection_rule": "direct argmax of each frozen visual posterior; fixed OCR-caption-joint tie break",
        "evaluation_unit": "40 development root sample ids",
        "metrics": ["expected_terminal_value", "support", "expected_strict", "mode_strict"],
        "controls": ["no_credit", "local_ig_m", "outcome"],
        "go_gates": {
        ...
        "input_paths": {
            "visual_value_freeze": str(query_freeze_path),
        ...
        "input_hashes": {
            "visual_value_freeze": sha256(query_freeze_path),
        ...
        "current_visual_targets_fit_to_private_development_labels": False,
    ...
    output_path = output_dir / "DAGIG_V6_VISUAL_SELECTOR_DEVELOPMENT_PROTOCOL_FREEZE.json"
