#!/usr/bin/env python3
"""Train deterministic categorical evidence controls with bounded microbatches."""
...
def field_token_mask(tokenizer: Any, completion: str) -> tuple[list[int], list[int]]:
    ...
    if set(parsed) != {"action"} or parsed["action"] not in "ABCDE" or len(parsed["action"]) != 1:
        raise ValueError("categorical completion must contain exactly one A-E action")
    ...
    if sum(mask) != 1:
        raise ValueError("categorical action must optimize exactly one token")
...
def build_batch(
    ...
    return {"input_ids": input_ids.cuda(), "attention_mask": attention.cuda()}, field_masks[:, 1:].cuda(), slices
...
def save_checkpoint(
    ...
    torch.save(payload, temporary / "optimizer_and_rng.pt")
    state = {
        "method": method,
...
def main() -> None:
    ...
    allowed_protocols = {"dagig_v6_backward_evidence_explicit_categorical_deterministic_microbatch_v3"}
    if freeze.get("protocol_version") not in allowed_protocols:
        raise ValueError("batched evidence training v2 is not frozen")
    ...
    rows = read_jsonl(Path(freeze["input_paths"]["categorical_train_data"]))
    rows = rows[: args.max_groups] if args.max_groups else rows
    if not rows or any(
        len(row["completions"]) != 5
        or row.get("categorical_action_labels") != list("ABCDE")
        or [json.loads(value)["action"] for value in row["completions"]] != list("ABCDE")
        for row in rows
    ):
    ...
    if group_batch_size * accumulation_batches != 8:
        raise ValueError("effective batch must remain eight groups")
    checkpoint_dir = args.checkpoint_dir.resolve() if args.checkpoint_dir else None
    if checkpoint_dir is None:
        raise ValueError("categorical evidence training requires an atomic checkpoint directory")
    freeze_hash = sha256(freeze_path)
    checkpoint_state = (
        load_checkpoint_state(checkpoint_dir, args.method, freeze_hash, len(rows), group_universe_sha256)
        if checkpoint_dir
        else None
    )
    ...
    if checkpoint_state is not None and not args.resume:
        raise ValueError("checkpoint exists; pass --resume or use a new checkpoint directory")
    if args.stop_after_epoch and (not args.max_groups or not 1 <= args.stop_after_epoch < int(config["epochs"])):
        raise ValueError("--stop_after_epoch is allowed only for a bounded smoke before the final epoch")
    ...
    with torch.inference_mode():
        for start in range(0, len(rows), group_batch_size):
            ...
            if min(start + group_batch_size, len(rows)) % 500 < group_batch_size:
    ...
    # The reference-corrected policy requires current and reference logits to
    # use the same deterministic function. Eval mode disables LoRA dropout but
    # does not disable autograd.
    model.eval()
    if any(module.training for module in model.modules() if isinstance(module, torch.nn.Dropout)):
        raise ValueError("dropout remains active in deterministic policy training")
        ...
        if checkpoint_dir is not None:
            ...
            print(
                json.dumps(
                    {
                        "method": args.method,
                        "checkpoint_completed_epoch": epoch + 1,
                        "optimizer_steps": optimizer_steps,
                        "checkpoint_dir": str(checkpoint_dir),
                    }
                ),
            ...
            if args.stop_after_epoch == epoch + 1:
                print(json.dumps({"method": args.method, "intentional_smoke_stop_after_epoch": epoch + 1}), flush=True)
                return
    ...
    expected_steps = int(config["epochs"]) * ((len(rows) + int(config["effective_groups_per_optimizer_step"]) - 1) // int(config["effective_groups_per_optimizer_step"]))
    result = {
    ...
    audit_path = output_temporary / "DAGIG_V6_BACKWARD_EVIDENCE_POLICY_TRAIN_AUDIT.json"
