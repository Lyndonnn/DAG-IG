#!/usr/bin/env python3
"""One-shot internal audit for the frozen pairwise evidence-state value v2."""
...
METHODS = ("no_credit", "local_ig_m", "outcome", "old_dagig", "pairwise_dagig")
...
def bootstrap(rows: list[dict[str, Any]], metric: str, baseline: str) -> dict[str, float | int]:
    grouped: dict[str, list[float]] = defaultdict(list)
    for row in rows:
        grouped[row["sample_id"]].append(row["methods"]["pairwise_dagig"][metric] - row["methods"][baseline][metric])
    sample_ids = sorted(grouped)
    rng = random.Random(f"pairwise-evidence-v2:{metric}:{baseline}:20260720")
    draws = []
    for _ in range(5000):
        sampled = [sample_ids[rng.randrange(len(sample_ids))] for _ in sample_ids]
        draws.append(mean(value for sample_id in sampled for value in grouped[sample_id]))
...
def main() -> None:
    ...
    if train.get("decision") != "DAGIG_V6_PAIRWISE_EVIDENCE_STATE_CRITIC_V2_TRAIN_OOF_GO":
        raise ValueError("pairwise evidence-state critic did not pass train OOF")
    if freeze["input_hashes"]["auditor"] != sha256(Path(__file__).resolve()):
        raise ValueError("v2 auditor changed after freeze")
    if train["input_hashes"]["freeze"] != sha256(freeze_path):
        raise ValueError("v2 train audit came from another freeze")
    for key, raw_path in train["output_paths"].items():
        if sha256(Path(raw_path)) != train["output_hashes"][key]:
            raise ValueError(f"v2 train output changed: {key}")
    ...
    if len(predictions) != 2975 or len(categorical) != 595:
        raise ValueError(f"v2 internal universe mismatch: {len(predictions)}/{len(categorical)}")
    ...
    for method in METHODS:
        ...
        summary[method] = {
            ...
            "pairwise_value": mean(row["pairwise_value"] for row in rows),
    ...
    for baseline in ("no_credit", "local_ig_m", "outcome", "old_dagig"):
        comparisons[f"pairwise_dagig_vs_{baseline}"] = {
            "top_action_disagreement_rate": mean(
                row["methods"]["pairwise_dagig"]["evidence_action_id"] != row["methods"][baseline]["evidence_action_id"]
                for row in private_rows
            ),
            **{metric: bootstrap(private_rows, metric, baseline) for metric in ("support", "expected_strict", "mode_strict")},
        }
    threshold = freeze["development_gates"]
    dag, no_credit, local, outcome = summary["pairwise_dagig"], summary["no_credit"], summary["local_ig_m"], summary["outcome"]
    gates = {
        ...
        "differs_from_outcome": comparisons["pairwise_dagig_vs_outcome"]["top_action_disagreement_rate"] >= threshold["top_action_disagreement_vs_outcome_min"],
    ...
    decision = "DAGIG_V6_PAIRWISE_EVIDENCE_STATE_CRITIC_V2_DEVELOPMENT_GO" if all(gates.values()) else "DAGIG_V6_PAIRWISE_EVIDENCE_STATE_CRITIC_V2_DEVELOPMENT_NO_GO"
    output = args.output_dir.resolve()
    output.mkdir(parents=True, exist_ok=False)
    rows_path = output / "v6_pairwise_evidence_state_internal_private.jsonl"
    ...
    audit_path = output / "DAGIG_V6_PAIRWISE_EVIDENCE_STATE_CRITIC_V2_DEVELOPMENT_AUDIT.json"
