#!/usr/bin/env python3
"""Require microbatch deterministic No-credit to remain a fixed point."""
...
def main() -> None:
    ...
    if freeze.get("protocol_version") != "dagig_v6_backward_evidence_explicit_categorical_deterministic_microbatch_v3":
        raise ValueError("fixed-point audit requires deterministic categorical microbatch v3")
    if smoke.get("decision") != "DAGIG_V6_BACKWARD_EVIDENCE_POLICY_SMOKE_READY" or smoke.get("method") != "no_credit":
    ...
    decision = "DAGIG_V6_CATEGORICAL_NO_CREDIT_FIXED_POINT_GO" if all(gates.values()) else "DAGIG_V6_CATEGORICAL_NO_CREDIT_FIXED_POINT_NO_GO"
    result = {
        "decision": decision,
        "gates": gates,
