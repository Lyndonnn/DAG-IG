#!/usr/bin/env python3
"""Score one deterministic backward-query adapter on matched actions."""
...
def main() -> None:
    ...
    if freeze.get("protocol_version") != "dagig_v6_backward_fixed_descendants_equal_query_training_deterministic_v2":
        raise ValueError("query scorer requires deterministic v2")
    if sha256(Path(__file__).resolve()) != freeze["runner_hashes"]["scorer"]:
        raise ValueError("query scorer differs from frozen runner")
