#!/usr/bin/env python3
"""Score frozen reference/DAG-IG evidence policies on fresh query states."""
...
def field_token_mask(tokenizer: Any, value: str) -> tuple[list[int], list[int]]:
    ...
    if set(parsed) != {"selected_evidence_ids"}:
        raise ValueError("fresh evidence completion schema changed")
    ...
    if not any(mask):
        raise ValueError("empty fresh evidence field mask")
...
def build_batch(
    ...
    for rows in groups:
        prompt = build_evidence_selection_prompt(rows[0])
        ...
        begin = len(sequences)
        for row in rows:
            tokens, field_mask = field_token_mask(tokenizer, completion(row))
            sequence = prefix + tokens + [tokenizer.eos_token_id]
            mask = [0] * len(prefix) + field_mask + [0]
            if len(sequence) > max_tokens:
                raise ValueError(f"fresh evidence scoring sequence too long: {row['query_id']}")
...
def main() -> None:
    ...
    parser.add_argument("--group_batch_size", type=int, default=2)
    ...
    gates = {
        ...
        "selected_evidence_ids_field_only": True,
