...
def main() -> None:
    ...
    for group in groups:
        ...
        if group["partition"] == "policy_train":
            ...
            local_top.append(dag_top == max(range(len(local)), key=local.__getitem__))
    ...
    metrics = {
        ...
        "local_nonconstant_group_rate_train": mean(
            max(group["local_values"]) - min(group["local_values"]) > 1e-12 for group in train_groups
        ),
    ...
    gates = {
        ...
        "dagig_train_terminal_noninferior_local": metrics["train_direct_selector_terminal"]["dagig"] >= metrics["train_direct_selector_terminal"]["local_ig"] - 0.002,
    ...
    decision = "DAGIG_V6_FULL_QUERY_VALUES_V1_FROZEN" if all(gates.values()) else "DAGIG_V6_FULL_QUERY_VALUES_V1_NO_GO"
    output_dir = args.output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=False)
    train_path = output_dir / "v6_full_query_targets_v1_train.jsonl"
    internal_path = output_dir / "v6_full_query_targets_v1_internal_no_labels.jsonl"
    diagnostics_path = output_dir / "v6_full_query_value_v1_diagnostics_no_gold.jsonl"
    excluded_path = output_dir / "v6_full_query_v1_excluded_public.jsonl"
    for path, rows in (
    ...
    result = {
        "decision": decision,
        "protocol_version": "dagig_v6_full_real_search_uniform_intervention_query_backup_v1",
        "downstream_contract": {
    ...
    audit_path = output_dir / "DAGIG_V6_FULL_QUERY_VALUE_V1_FREEZE.json"
