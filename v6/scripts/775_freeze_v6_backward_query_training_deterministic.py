#!/usr/bin/env python3
"""Freeze deterministic equal-compute query training after evidence GO."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--trainer",
        type=Path,
        default=scripts / "776_train_v6_backward_query_policy_deterministic.py",
    )
    parser.add_argument(
        "--scorer",
        type=Path,
        default=scripts / "777_score_v6_backward_query_policy_deterministic.py",
    )
    parser.add_argument(
        "--train_fit_auditor",
        type=Path,
        default=scripts / "778_audit_v6_backward_query_train_fit_deterministic.py",
    )
    parser.add_argument(
        "--fixed_point_auditor",
        type=Path,
        default=scripts / "779_audit_v6_query_no_credit_fixed_point.py",
    )
    parser.add_argument("--categorical_handoff_audit", type=Path, required=True)
    parser.add_argument(
        "--fresh_evidence_scorer",
        type=Path,
        default=scripts / "780_score_v6_backward_query_fresh_evidence_policy_categorical.py",
    )
    parser.add_argument(
        "--preflight_failure_auditor",
        type=Path,
        default=scripts / "782_audit_v6_query_preflight_runtime_failure.py",
    )
    ...
    paths = {
        ...
        "train_fit_auditor": args.train_fit_auditor.resolve(),
        "fixed_point_auditor": args.fixed_point_auditor.resolve(),
        "categorical_handoff_audit": args.categorical_handoff_audit.resolve(),
        "fresh_evidence_scorer": args.fresh_evidence_scorer.resolve(),
        "preflight_failure_auditor": args.preflight_failure_auditor.resolve(),
    }
    control = read_json(paths["control_freeze"])
    format_audit = read_json(paths["initializer_format_audit"])
    handoff = read_json(paths["categorical_handoff_audit"])
    ...
    if handoff.get("decision") != "DAGIG_V6_QUERY_CATEGORICAL_EVIDENCE_HANDOFF_GO":
        raise ValueError("categorical evidence handoff is not GO")
    for key, path in handoff["input_paths"].items():
        if sha256(Path(path)) != handoff["input_hashes"][key]:
            raise ValueError(f"categorical evidence handoff input changed: {key}")
    if Path(handoff["input_paths"]["fresh_categorical_scorer"]).resolve() != paths["fresh_evidence_scorer"]:
        raise ValueError("handoff audit belongs to another fresh categorical scorer")
    ...
    if not all(paths[key].is_file() for key in ("trainer", "scorer", "train_fit_auditor", "fixed_point_auditor", "fresh_evidence_scorer", "preflight_failure_auditor")):
        raise ValueError("deterministic query runner set is incomplete")
    ...
    result = {
        "decision": decision,
        "protocol_version": "dagig_v6_backward_fixed_descendants_equal_query_training_deterministic_v2",
        ...
        "runner_hashes": {
            ...
            for key in ("trainer", "scorer", "train_fit_auditor", "fixed_point_auditor", "fresh_evidence_scorer", "preflight_failure_auditor")
        },
        "matched_controls": {
    ...
    audit_path = output / "DAGIG_V6_BACKWARD_QUERY_DETERMINISTIC_TRAINING_FREEZE.json"
