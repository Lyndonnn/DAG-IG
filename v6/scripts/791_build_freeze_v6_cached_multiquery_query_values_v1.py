#!/usr/bin/env python3
"""Build query-node values under the frozen direct DAG-IG evidence selector."""
...
def main() -> None:
        ...
        diagnostics.append(
            {
                ...
                "local_modal_values": group["local_values"],
    ...
    metrics = {
        ...
        "dagig_local_top_agreement": mean(local_top),
        "dagig_outcome_top_agreement": mean(outcome_top),
    ...
    gates = {
        ...
        "dagig_local_identifiable": metrics["dagig_local_mean_tv"] >= 0.01 and metrics["dagig_local_top_agreement"] <= 0.95,
        "dagig_outcome_identifiable": metrics["dagig_outcome_mean_tv"] >= 0.01 and metrics["dagig_outcome_top_agreement"] <= 0.95,
    ...
    decision = "DAGIG_V6_CACHED_MULTIQUERY_QUERY_VALUES_FROZEN" if all(gates.values()) else "DAGIG_V6_CACHED_MULTIQUERY_QUERY_VALUES_NO_GO"
    output_dir = args.output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=False)
    train_path = output_dir / "v6_cached_multiquery_query_targets_train.jsonl"
    internal_path = output_dir / "v6_cached_multiquery_query_targets_internal_no_labels.jsonl"
    diagnostics_path = output_dir / "v6_cached_multiquery_query_value_diagnostics_no_gold.jsonl"
    excluded_path = output_dir / "v6_cached_multiquery_query_excluded_public.jsonl"
    for path, rows in (
    ...
    result = {
        "decision": decision,
        "protocol_version": "dagig_v6_cached_multiquery_hard_evidence_query_backup_v1",
        "downstream_contract": {
        ...
        "method_contract": {
            ...
            "local_ig": "modal answer child value under the same selected evidence action, KL-matched on train",
    ...
    audit_path = output_dir / "DAGIG_V6_CACHED_MULTIQUERY_QUERY_VALUE_FREEZE.json"
