#!/usr/bin/env python3
"""Run the preregistered full-query development selector audit."""
...
def main() -> None:
    ...
    if protocol.get("decision") != "DAGIG_V6_FULL_QUERY_SELECTOR_DEVELOPMENT_PROTOCOL_FROZEN":
        raise ValueError("full-query development protocol is not frozen")
    ...
    if len(internal_rows) != 120 or len(diagnostics) != 120:
        raise ValueError(f"expected 120 development visual parents, found {len(internal_rows)}/{len(diagnostics)}")
    ...
    for public in internal_rows:
        ...
        if not 3 <= len(public["actions"]) <= 5 or len(public["actions"]) != len(diagnostic["query_action_ids"]):
    ...
    gates = {
        "complete_120_development_visual_parents": len(private_rows) == 120,
    ...
    decision = "DAGIG_V6_FULL_QUERY_SELECTOR_DEVELOPMENT_GO" if all(gates.values()) else "DAGIG_V6_FULL_QUERY_SELECTOR_DEVELOPMENT_NO_GO"

    output_dir = args.output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=False)
    private_path = output_dir / "v6_full_query_selector_development_private.jsonl"
    ...
    result = {
        ...
        "full_query_targets_fit_to_private_development_labels": False,
    ...
    audit_path = output_dir / "DAGIG_V6_FULL_QUERY_SELECTOR_DEVELOPMENT_AUDIT.json"
    ...
    lines = [
        "# Full Multi-Action Query Selector Development Audit",
        "",
        "## Scope",
        "",
        "Development evaluation of the frozen 3-to-5-action query universe under the direct DAG-IG evidence selector and frozen answer policy. The same 40 samples were previously consumed by a narrower diagnostic, so this is not paper-final evidence. No training, new search, dev, or test was used.",
        "",
        "## Results",
        "",
    ...
    lines.append(
        ...
        if decision == "DAGIG_V6_FULL_QUERY_SELECTOR_DEVELOPMENT_GO"
        else "Do not train or open dev/test. Revise the query candidate/action protocol, not model capacity."
    )
    report_path = output_dir / "DAGIG_V6_FULL_QUERY_SELECTOR_DEVELOPMENT_REPORT.md"
