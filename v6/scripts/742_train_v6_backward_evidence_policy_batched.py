#!/usr/bin/env python3
"""Batched, semantically equivalent trainer for backward evidence controls."""
...
def field_token_mask(tokenizer: Any, completion: str) -> tuple[list[int], list[int]]:
    ...
    if not any(mask):
        raise ValueError("empty optimized field mask")
...
def main() -> None:
    ...
    allowed_protocols = {
        "dagig_v6_backward_fixed_answer_equal_evidence_training_batched_v2",
        "dagig_v6_backward_fixed_answer_equal_evidence_training_batched_six_epoch_v3",
        "dagig_v6_backward_fixed_answer_equal_evidence_training_batched_six_epoch_resumable_v4",
    ...
    if freeze.get("protocol_version", "").endswith("resumable_v4") and checkpoint_dir is None:
        raise ValueError("resumable v4 requires --checkpoint_dir")
